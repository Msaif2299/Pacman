...
def pac_life_reducer(mode, pac, ghosts, gameDisplay, score, frightenedTimeLeft, ghosts_combo, direction, game, t):
    if mode == 'game':
...
def getNextSprite(direction, count, pac_sprite):
        angle = 0
        prev_count = count
        count += 1
        if count == 3:
...
if __name__ == '__main__':
    ...
    for i in range(height):
        ...
        grid.append(temp)
    pac = getNextSprite('right', count, pac_sprite)
